...
class SyscoinConfig():
    ...
    def slurp_config_file(self, filename):
        # read syscoin.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is syscoin.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        # standard Syscoin defaults...
        default_port = 8370 if (network == 'mainnet') else 18370

        # use default port for network if not specified in syscoin.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from syscoin.conf
