...
class SyscoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some syscoind methods
    ...
    @classmethod
    def from_syscoin_conf(self, syscoin_dot_conf):
        from syscoin_config import SyscoinConfig
        config_text = SyscoinConfig.slurp_config_file(syscoin_dot_conf)
        creds = SyscoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def we_are_the_winner(self):
        ...
        winner = syscoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = syscoinlib.blocks_to_seconds(diff)
