...
class GovernanceObject(BaseModel):
    ...
    def import_gobject_from_syscoind(self, syscoind, rec):
        ...
        # shim/syscoind conversion
        object_hex = syscoinlib.SHIM_deserialise_from_syscoind(object_hex)
        objects = syscoinlib.deserialise(object_hex)
        subobj = None
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SyscoinDrive, etc.)
        return False
    ...
    def prepare(self, syscoind):
        try:
            object_hash = syscoind.rpc_command(*self.get_prepare_command())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SYSCOIND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, syscoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - syscoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, syscoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - syscoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, syscoind):
        now = int(time.time())
        return (self.created_at < (now - syscoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, syscoind):
        if self.is_expired(syscoind):
            return False

        return True

    def is_deletable(self, syscoind):
        if self.is_expired(syscoind):
            return True
